...
public class TaskServiceImpl extends BaseService implements ITaskService {
	...
	public int deleteTaskById(Long taskId, String userId) {
		TaskInfo taskInfo = taskInfoMapper.selectByPrimaryKey(taskId, userId);
		List<JUserTask> userTask = jUserTaskMapper.selectWantDeleteTaskUser(taskId);
		if (taskInfo != null) {
			if (taskInfo.getResId() != null) {
				List<TaskInfo> paper = taskInfoMapper.selectPaperAndRes(null,taskInfo.getResId());
				if (paper.size() == 1) {
					resourceInfoMapper.updateStatus(taskInfo.getResId(), 0);
				}
			}
			if (taskInfo.getPaperId() != null) {
				List<TaskInfo> res = taskInfoMapper.selectPaperAndRes( taskInfo.getPaperId(),null); 
				if (res.size() == 1) {
					paperInfoMapper.updateStatus(taskInfo.getPaperId(), 0);
				}
			}
		}
		for (JUserTask jUserTask : userTask) {
			jUserTaskMapper.deleteByPrimaryKey(jUserTask.getRef());
		}
